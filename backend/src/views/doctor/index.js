  ...
  const columns = [
    ...
    },
    {
      ...
      options: {
        ...
        sort: false,
        customBodyRender: (_, { rowIndex }) => {
          const { top_dr, id } = datatableData[rowIndex]
          ...
          return (
            <Switch
              ...
              onChange={() => {
                if (userRole == 1) {
                  const data = { id: id, top_dr: !top_dr }
                  updateTopDoctorStatus(data, id)
                    .then(() => {
                      toast.success('status changed successfully!', {
                        key: data.id,
                      })
          ...
          return (
            <div>
              ...
              <Icons.RemoveRedEye
                className="viewIcon"
                onClick={() => {
                  const doctorData = datatableData.find((data) => data.id === value)
                  navigate('/doctor/view', {
                    state: { doctorData: doctorData, baseurl: baseurl },
                  })
                }}
              />
            </div>
